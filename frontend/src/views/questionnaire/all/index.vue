      ...
      <div v-if="value === 'designer'">
        <el-table :data="designer" border stripe style="width: 100%">
          ...
          <el-table-column align="center" prop="starFlag" label="星标" width="80">
            <template #default="scope">
              <el-icon v-if="scope.row.starFlag === '1'" color="#E6A23C">
                <StarFilled/>
              </el-icon>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="操作">
            <template #default="scope">
              <el-button type="text" size="small">
                <a @click="gotoDetail(scope.row.answerId)">编辑</a>
              </el-button>
...
import {listUserQuestionnaire, deleteQuestionnaire} from '@/api/question/question';
...
const getUser = () => {
  listUserAnswers(store.getters.userId, "all").then(res => {
    user.value = []
    console.log(res)
    for (let i in res.data) {
      // console.log(res.data[i])
      user.value.push({
        answerId: res.data[i].answer.answerId,
        answerStatus: res.data[i].answer.answerStatus,
        anonymousFlag: res.data[i].questionnaire.anonymousFlag,
        createTime: res.data[i].answer.createTime,
        introduction: res.data[i].questionnaire.introduction,
        modifyFlag: res.data[i].questionnaire.modifyFlag,
        questionnaireId: res.data[i].answer.questionnaireId,
        questionnaireStatus: res.data[i].questionnaire.questionnaireStatus,
        starFlag: res.data[i].answer.starFlag,
        title: res.data[i].questionnaire.title,
        fillIp: res.data[i].answer.fillIp,
